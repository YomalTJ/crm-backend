  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < queries.length; i++) {
        ...
        try {
          ...
          if (error.code === 'ER_DUP_ENTRY') {
            ...
            console.log(
              `⚠️  Table doesn't exist for query ${i + 1}, skipping...`,
            );
          } else if (error.code === 'ER_CANT_DROP_FIELD_OR_KEY') {
            console.log(
              `⚠️  Cannot drop field/key for query ${i + 1}, skipping...`,
            );
          } else {
            console.warn(
              `⚠️  Non-critical error in query ${i + 1}:`,
              error.message,
            );
      ...
      if (fileError.code === 'ENOENT') {
        console.log(
          '⚠️  seed-gnd-data.sql file not found, skipping seeding...',
        );
      } else {
        throw fileError;
      }
